...
import { getAllAssetsByName } from '../store/actions/AssetActions';
import { Link } from 'react-router-dom';
import img1 from '../images/Dell-Laptop.png'; 
import { useParams } from 'react-router-dom';

function FetchAllAssetsByName(){
     const assets=useSelector(state=>state.assetReducer.name)
     const { name } = useParams();
    const dispatch=useDispatch();
    const myFunction=()=>{
        dispatch(getAllAssetsByName(name));
    }
    useEffect(()=> {
        myFunction();
    },[name]);
...
export default FetchAllAssetsByName;