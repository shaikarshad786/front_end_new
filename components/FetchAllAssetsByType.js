...
import { getAllAssetsByType } from '../store/actions/AssetActions';
import { Link } from 'react-router-dom';
import img1 from '../images/Dell-Laptop.png'; 
import { useParams } from 'react-router-dom';

function FetchAllAssetsByType(){
     const assets=useSelector(state=>state.assetReducer.type)
     const { type } = useParams();
    const dispatch=useDispatch();
    const myFunction=()=>{
        dispatch(getAllAssetsByType(type));
    }
    useEffect(()=> {
        myFunction();
    },[type]);  
...
export default FetchAllAssetsByType;